...
def test_login_with_dni_success(mock_create_token, mock_verify_password_hash, mock_session):
    ...
    # Configuración de los retornos de los mocks
...
@patch("app.modules.auth.service.verify_password_hash")
def test_login_with_dni_invalid_password(mock_verify_password_hash, mock_session):
    login_data = LoginRequest(
        document="05302528",
    ...
    mock_session.query.return_value.filter.return_value.first.return_value = mock_user
    mock_verify_password_hash.return_value = False
